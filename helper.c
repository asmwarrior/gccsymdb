...
def (const char *root_fn, const char *def)
{
  ...
  dyn_string_copy_cstr (gbuf,
			"select fileName, position, flag from Helper where defName = '");
...
caller (const char *root_fn, const char *def)
{
  ...
  const char *fid = dep_get_fid (root_fn);
  dyn_string_copy_cstr (gbuf,
			"select fileName, position, defName, flag from Helper where defID in (");
  dyn_string_append_cstr (gbuf,
...
static void
addsym (const char *root_fn, const char *def, const char *position)
{
  const char *fid = dep_get_fid (root_fn);
  if (fid == NULL)
    {
  ...
  dyn_string_append_cstr (gbuf, ", ");
  dyn_string_append_cstr (gbuf, position);
  dyn_string_append_cstr (gbuf, ");");
  db_error ((sqlite3_exec (db, dyn_string_buf (gbuf), NULL, 0, NULL)));
...
static void
rmsym (const char *root_fn, const char *def, const char *position)
{
  int nrow, ncolumn;
  char *error_msg, **table;
  const char *fid = dep_get_fid (root_fn);
  ...
  dyn_string_append_cstr (gbuf, " and position = ");
  dyn_string_append_cstr (gbuf, position);
  dyn_string_append_cstr (gbuf, ";");
  db_error (sqlite3_get_table (db, dyn_string_buf (gbuf), &table,
			       &nrow, &ncolumn, &error_msg));
